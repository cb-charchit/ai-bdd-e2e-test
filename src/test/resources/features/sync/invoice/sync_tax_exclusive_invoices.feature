...
Feature: Sync Invoices with Tax Exclusive Plans to Quickbooks
  ...
  Scenario: Rajat syncs ChargeBee invoice to QuickBooks with a FlatFee Pricing Model
    ...
    And he has a customer "US-Region-Customer" belong to "tax exclusive region"
    And he has a <item> with <amount> and currency "USD"
      | item           | amount |
      | FlatFee-Plan   | 100    |
      | FlatFee-Addon  | 10     |
      | FlatFee-Charge | 10     |
    And he creates a new subscription for "US-Region-Customer" with the following values
