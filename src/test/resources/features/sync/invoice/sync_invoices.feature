...
Feature: Sync Invoices to Quickbooks
    ...
    Scenario: Rajat syncs ChargeBee invoices to QuickBooks
      ...
      And he has a <item_type> with <unit_amount> and <quantity> and currency "USD"
        ...
        | flat_fee_plan          | 100         | 1        |
        | ff_recurring_addon     | 10          | 2        |
        | ff_non_recurring_addon | 10          | 2        |
       And he creates a "new" subscription for "John" with the following values
         | item_price_ids | flat_fee_plan-USD-Monthly,ff_non_recurring_addon-USD-Monthly |
